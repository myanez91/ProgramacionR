...
```{r}
# vemos los primeros registros PUB_NOM_ACTECOS
...
Para nuestro análisis de la data de PUB_NOM_DOMICILIO: necesitaremos los campos RUT, FECHA y REGION con estado de VIGENCIA= S "están vigentes"

```{r}
PUB_NOM_DOMICILIO2 <- PUB_NOM_DOMICILIO %>%
  filter(VIGENCIA == "S") %>%
  select(RUT, FECHA, REGION)
```
...
-- Eliminamos REGION en los datos de REGION ya que se representa por el nombre de la columna y facilita su visualización.
...
```{r}
Data_Studio <- subset(Data_Studio, REGION != "Sin Información")
```
...
data_REG_Div <- Data_Studio |> 
  dplyr::group_by(REGION, Division) |> 
  dplyr::summarise(n = n(), .groups = 'drop')

ggplot(data = data_REG_Div, aes(x = REGION, y = n, fill = REGION)) +
...
Segun la grafica podriamos inferir que la gran concentración de empresas están el la XIII Región, independiente de su division.
